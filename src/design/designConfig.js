...
const getSurfaceSx = ({ surface = "glass", backgroundColor = "", borderColor = "", accentColor = "" } = {}) => {
  ...
  if (surface === "soft") {
    return {
      background: backgroundColor || "rgba(255,255,255,0.42)",
      border: `1px solid ${borderColor || "rgba(255,255,255,0.48)"}`,
  ...
  return (
    <Button
      ...
      sx={{
        ...
        px: 2,
        minHeight: 42,
        color: variant === "contained" ? "#fff" : toneColor,
        borderColor: toneColor,
        background: variant === "contained" ? `linear-gradient(180deg, ${toneColor}, ${toneColor}dd)` : undefined,
        position: "relative",
        overflow: "hidden",
      }}
    >
...
function RecentVodsRenderer({ title, subtitle, count = 4, showButton = true, surface = "glass", width, customMaxWidth, minHeight, padding }) {
...
export const designConfig = {
  root: {
    ...
    render: ({ children, ...props }) => {
      const background =
        props.backgroundMode === "image" && safeUrl(props.backgroundImage)
          ? `linear-gradient(rgba(226,233,243,0.74), rgba(226,233,243,0.88)), url(${safeUrl(props.backgroundImage)}) center / cover fixed`
          : props.backgroundMode === "color"
            ? props.backgroundColor || "var(--soft-bg)"
            : props.backgroundMode === "gradient"
              ? props.backgroundGradient || "var(--soft-bg)"
              : "transparent";
      ...
      return (
        <Box
          ...
          sx={{
            minHeight: "100%",
            background,
            color: props.textColor || "var(--soft-text)",
  ...
  categories: {
    layout: {
      title: "Layout",
      components: ["HeroDirectory", "DirectoryGrid", "TextBlock", "ButtonRow", "Spacer"],
  ...
  components: {
      ...
      render: (props) => {
        ...
        const textContent = (
          <Box sx={{ minWidth: 0, textAlign: isCentered ? "center" : "left" }}>
            ...
            {props.subtitle && (
              <Typography variant="h5" sx={{ mt: 0.85, color: "text.secondary", fontSize: { xs: "1rem", md: "1.2rem" }, lineHeight: 1.3 }}>
                {props.subtitle}
              </Typography>
            )}
            {props.body && (
              <Typography variant="body1" sx={{ mt: 1.25, color: "text.secondary", maxWidth: isCentered ? 760 : 560, mx: isCentered ? "auto" : 0, lineHeight: 1.55 }}>
                {props.body}
              </Typography>
            )}
    ...
    DirectoryGrid: {
      ...
      render: (props) => {
        ...
        return (
          <Box className={props.customClassName} sx={getResizeSx(props)}>
      ...
      render: (props) => (
        <Box
          ...
          sx={{
            ...
            minHeight: Number(props.minHeight) > 0 ? Number(props.minHeight) : undefined,
            ...getSurfaceSx({ surface: props.surface }),
        ...
        >
          ...
          {props.title && (
            <Typography variant="h4" sx={{ color: "primary.main", lineHeight: 1.05 }}>
              {props.title}
            </Typography>
          )}
          {props.body && (
            <Typography component="div" variant="body1" sx={{ color: "text.secondary", whiteSpace: "pre-line", mt: props.title ? 1 : 0, lineHeight: 1.58 }}>
              {props.body}
            </Typography>
          )}
        </Box>
    ...
    ImageBlock: {
      ...
      render: (props) => (
        <Box className={getAnimationClass(props.animation, props.customClassName)} sx={{ ...getWidthSx(props.width, props.customMaxWidth), minHeight: Number(props.minHeight) > 0 ? Number(props.minHeight) : undefined, mx: "auto" }}>
          {sectionHeading(props.title, props.subtitle)}
